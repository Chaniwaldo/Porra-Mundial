document.addEventListener('DOMContentLoaded', () => {
  ...
  const EXTRA_DAYS = [{ key: 'bonus', label: 'Jornada 0 · Bonus del torneo', sort: '2026-06-01' }];
    ...
    snap.forEach(doc => {
      ...
      if (pred.type === 'bonus-full') {
        ...
        [pred.finalist1, pred.finalist2].forEach(f => {
          if (f && officialFinals.some(of => containsApprox(f, of))) pts += 10;
        });
        ...
        const officialBest = bonusOfficial.best11 || [];
        (pred.best11 || []).forEach(name => {
          if (name && officialBest.some(of => containsApprox(name, of))) pts += 3;
        });
        row.points += pts;
        row.bonus += pts;
        return;
